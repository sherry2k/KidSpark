...
const PuzzleGame: React.FC<PuzzleGameProps> = ({ progress, onBack, onComplete }) => {
  ...
  const handleLevelComplete = useCallback(() => {
    const totalLevels =
      puzzleType === 'shape-match'
        ? SHAPE_MATCH_LEVELS.length
        : puzzleType === 'pattern'
          ? PATTERN_LEVELS.length
          : puzzleType === 'size-sort'
    ...
    if (isLastLevel) {
      // Show full celebration only on final level
      setShowCelebration(true);
      setTimeout(() => {
        onComplete(3);
      }, 500);
    } else {
      // Show "Great Job!" text briefly for non-final levels
      ...
      setTimeout(() => {
        ...
        if (puzzleType === 'size-sort') {
          const nextLevel = SIZE_SORT_LEVELS[currentLevel + 1];
          if (nextLevel) {
            const shuffled = [...nextLevel.items].sort(() => Math.random() - 0.5);
            setAvailableItems(shuffled);
          }
        }
      }, 1000);
  ...
  // ========================================
  // LEVEL SUCCESS OVERLAY (non-final levels)
  // ========================================
  if (showLevelSuccess) {
    return (
      <GameBackground variant="game">
        <div className="h-full flex flex-col items-center justify-center px-4 relative">
          ...
          >
            <motion.div
              className="w-24 h-24 md:w-28 md:h-28 bg-green-500 rounded-full flex items-center justify-center shadow-2xl border-4 border-white mb-4"
              style={{
                boxShadow: '0 8px 0 #15803D, 0 12px 25px rgba(0,0,0,0.2)',
              }}
            ...
            <motion.h2
              className="text-3xl md:text-4xl font-bold text-green-600 drop-shadow-lg text-center"
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-4 overflow-y-auto py-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl w-full">
              ...
              >
                ...
                <h3 className="text-xl font-bold mb-1" style={{ fontFamily: "'Bubblegum One', cursive" }}>
                  Shape Match
                </h3>
                <p className="text-sm opacity-90">Match pictures to their pairs!</p>
              ...
              >
                ...
                <p className="text-sm opacity-90">Complete the pattern!</p>
                <div className="mt-3 flex justify-center gap-1">
                  <span>⭐⭐</span>
              ...
              >
                ...
                <p className="text-sm opacity-90">Arrange from small to big!</p>
                <div className="mt-3 flex justify-center gap-1">
                  <span>⭐⭐</span>
                  <span className="text-xs opacity-75">Medium</span>
                </div>
  ...
  // ========================================
  // CELEBRATION SCREEN (final level only)
  // ========================================
  if (showCelebration) {
    return (
      <GameBackground variant="game">
        <div className="h-full flex flex-col items-center justify-center px-4">
          ...
          >
            ...
            <p className="text-gray-600 mb-6">You solved all the levels!</p>
  ...
  if (puzzleType === 'shape-match') {
    ...
    const allMatched = level && Object.keys(matches).length === level.items.length;

    useEffect(() => {
      if (allMatched && !showCelebration && !showLevelSuccess) {
        handleLevelComplete();
      }
    }, [allMatched, showCelebration, showLevelSuccess, handleLevelComplete]);
                  ...
                  {level.items.map((item) => {
                    ...
                    return (
                      <motion.button
                        ...
                        style={{
                          boxShadow: isSelected ? '0 6px 0 #F59E0B' : '0 4px 0 rgba(0,0,0,0.1)',
                        }}
                  ...
                  {level.items.map((item) => {
                    ...
                    return (
                      <motion.button
                        ...
                        className={`aspect-square rounded-2xl flex items-center justify-center text-4xl md:text-5xl shadow-lg border-4 transition-all ${
                          isUsed ? 'bg-green-100 border-green-400' : 'bg-gray-200 border-white'
                        }`}
                        style={{
      ...
      <GameBackground variant="game">
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-4 pb-4">
            ...
            <p className="text-lg font-bold text-gray-700 mb-4" style={{ fontFamily: "'Bubblegum One', cursive" }}>
              Choose your answer:
            </p>
  ...
  if (puzzleType === 'size-sort') {
    ...
    const handleItemPick = (item: any) => {
      ...
      if (newSorted.length === level.items.length) {
        const isCorrectOrder = newSorted.every((item, i) => i === 0 || item.size >= newSorted[i - 1].size);
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-4 pb-4">
                  ...
                  return (
                    <div
                      ...
                      className={`aspect-square rounded-2xl flex items-center justify-center border-4 border-dashed ${
                        item ? 'bg-green-100 border-green-400' : 'bg-gray-100 border-gray-300'
                      }`}
                      style={{ minHeight: '60px' }}
                    >
                      {item && (
                        <motion.span className="text-4xl md:text-5xl" initial={{ scale: 0 }} animate={{ scale: 1 }}>
                          {item.emoji}
                        </motion.span>
                      )}
                    </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 w-full max-w-md">
              {availableItems.map((item, i) => (
                ...
                >
                  <span className="text-5xl md:text-6xl" style={{ fontSize: `${2 + item.size * 0.5}rem` }}>
                    {item.emoji}
                  </span>
                  <span className="text-xs font-bold text-gray-600 mt-1">{item.label}</span>
                </motion.button>
              ))}
            </div>
