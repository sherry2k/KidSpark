...
const QuizScreen: React.FC<QuizScreenProps> = ({ progress, onBack, onAnswer, onComplete }) => {
  ...
  const [wrongAnswers, setWrongAnswers] = useState<number[]>([]); // 🆕 Track wrong answers
  ...
  const handleAnswer = useCallback((index: number) => {
    ...
    if (correct) {
      ...
      setTimeout(() => {
        ...
        setWrongAnswers([]); // Reset wrong answers for next question

        if (currentQ < questions.length - 1) {
          setCurrentQ((q) => q + 1);
        } else {
          const starsEarned = score + 1;
      ...
      setTimeout(() => {
        setShowFeedback(false);
        setSelectedAnswer(null); // Reset so user can try again
        // Don't move to next question - stay here!
      }, 1500);
    }
            ...
            >
              ...
              >
                ...
                <p 
                  ...
                  style={{ fontFamily: "'Bubblegum One', cursive" }}
                >
                  {wrongAnswers.length > 0 ? '🤔 Try again!' : '👆 Choose your answer:'}
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  {question.options.map((option, i) => {
                    ...
                    const isWrongPreviously = wrongAnswers.includes(i); // 🆕 Was tried before?
                    ...
                    } else if (isWrongPreviously) {
                      // 🆕 Show previously wrong answers as disabled/gray
                      bgClass = 'bg-gray-300 text-gray-500 border-gray-300';
                    ...
                    return (
                      <motion.button
                        ...
                        style={{ 
                          ...
                          opacity: isWrongPreviously && !isSelected ? 0.5 : 1,
                        }}
                        initial={{ y: 20, opacity: 0, scale: 0 }}
                        animate={{ y: 0, opacity: isWrongPreviously && !isSelected ? 0.5 : 1, scale: 1 }}
                        transition={{ delay: 0.3 + i * 0.05, type: 'spring' }}
                        whileHover={!showResult && !isWrongPreviously ? { scale: 1.03, y: -3 } : {}}
                        whileTap={!showResult && !isWrongPreviously ? { 
                          scale: 0.95, 
                          y: 3,
                          boxShadow: `0 2px 0 ${currentShadow}, 0 4px 10px rgba(0,0,0,0.15)`
                        } : {}}
                        disabled={selectedAnswer !== null || isWrongPreviously}
                      >
                        {/* Letter/Result Badge */}
                        <motion.div
                          className={`w-12 h-12 md:w-14 md:h-14 rounded-full flex items-center justify-center text-xl md:text-2xl font-bold flex-shrink-0 border-3 ${
                            showResult && isSelected && isCorrect ? 'bg-white text-green-500' :
                            showResult && isSelected && !isCorrect ? 'bg-white text-red-500' :
                            isWrongPreviously ? 'bg-gray-100 text-gray-400' :
                        ...
                        <span 
                          ...
                          style={{ 
                            textShadow: showResult && isSelected && (isCorrect || !isCorrect) 
